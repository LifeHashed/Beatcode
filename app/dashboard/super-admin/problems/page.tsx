...
import { Edit, Trash2, Plus, Eye, Upload, BarChart3 } from 'lucide-react';
...
export default function SuperAdminProblemsPage() {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      const response = await fetch(`/api/super-admin/problems?${params}`);
  ...
  const handleSeedQuestions = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: `Seeded ${data.created} questions, skipped ${data.skipped} duplicates`,
        });
        fetchQuestions();
      }
    } catch (error) {
  ...
  const handleDeleteQuestion = async (questionId: string) => {
    if (!confirm('Are you sure you want to delete this question? This will affect all users.')) return;

    try {
      const response = await fetch(`/api/super-admin/problems/${questionId}`, {
        method: 'DELETE',
      });
  ...
  useEffect(() => {
    fetchQuestions();
  }, [currentPage, searchQuery, selectedDifficulty, sortBy]);

  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Super Admin - Problem Management</h1>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleSeedQuestions}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Total Problems</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-green-600">Easy</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-red-600">Hard</CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-muted-foreground space-y-1">
                        ...
                        <div>Added by: {question.addedBy.name} ({question.addedBy.email})</div>
